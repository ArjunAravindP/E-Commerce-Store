...
import { fetchOrders, getStructuredDate } from "../../util/hooks"
...
export default function AdminOrders() {
  ...
  return (
    <>
          ...
          <Fragment key={order._id}>
            <Grid onClick={() => handleClickOpen(order)} component={Paper} className={classes.orderitem} sx={{ minWidth: 275, maxHeight: 100, margin: 0 }}>
              <CardContent className={classes.content}>
                ...
                <p>Placed by: {order.billingName}</p>
                <p>
      ...
      <OrderDialog handleStatus={handleStatus} adminOrder={true} open={open} handleClose={handleClose} order={currentOrder} />
    </>
  )
}
